...
PROJECTS=$(find "$PARA_DIR/1-projects" -name "_index.md" 2>/dev/null | wc -l | tr -d ' ')
AREAS=$(find "$PARA_DIR/2-areas" -name "_index.md" 2>/dev/null | wc -l | tr -d ' ')
RESOURCES=$(find "$PARA_DIR/3-resources" -name "_index.md" 2>/dev/null | wc -l | tr -d ' ')
INBOX=$(find "$PARA_DIR/0-inbox" -name "*.md" 2>/dev/null | wc -l | tr -d ' ')

# Count overdue projects (deadline < today)
TODAY=$(date +%Y-%m-%d)
OVERDUE=0
for f in "$PARA_DIR/1-projects/"*/_index.md; do
  [ -f "$f" ] || continue
